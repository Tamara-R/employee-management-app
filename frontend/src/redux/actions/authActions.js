...
export const sendConfirmationEmail = (email) => async (dispatch) => {
  try {
    ...
    const {data} = await axios.post('/api/v1/account/email', email, config);

    dispatch({type: ACCOUNT_SUCCESS, payload: data});

  } catch (error) {
    dispatch({type: ACCOUNT_FAIL, payload: error.response.data.error});
  }
}
...
export const verifyUser = ( token ) => async (dispatch) => {
  try {
    ...
    const {data} = await axios.get(`/api/v1/account/verify/${token}`, config );

    dispatch({type: VERIFY_ACCOUNT_SUCCESS, payload: data});
    
  } catch (error) {

    dispatch({type: VERIFY_ACCOUNT_FAIL, payload: error.response.data.error});
  }
}
...
export const createUser= (userData) => async (dispatch) => {
  ...
  try {
      ...
      const {data} = await axios.post('/api/v1/admin/user/create', userData, config);

      dispatch({type: CREATE_USER_SUCCESS, payload: data.user});

  } catch (error) {
      console.log(error.response)
      dispatch({type: CREATE_USER_FAIL, payload: error.response.data.error});
  }
}
